...
public class Renderer {
    ...
    private static ThreadPool pool = new ThreadPool(3000, 6);
    ...
    public static void renderScene(Scene scene, Graphics gfx, int width, int height, float resolution) {
    	renderSceneExecService(scene, gfx, width, height, resolution);
    }
    ...
    public static void renderScenePoolCol(Scene scene, Graphics gfx, int width, int height, float resolution) {
    	...
    	//CountDownLatch doneSignal = new CountDownLatch(height);
    	int blockSize = (int) (1 / resolution);
        long start = System.currentTimeMillis();
        Latch latch = new Latch(width/blockSize);
    ...
    public static void renderScenePoolBuff(Scene scene, Graphics gfx, int width, int height, float resolution) {
    		...
    		Runnable r = ()->{
        		for (int y = 0; y<height; y+=blockSize) {
        			int yy = y;
                        float[] uv = getNormalizedScreenCoordinates(xx, yy, width, height);
                        PixelData pixelData = computePixelInfo(scene, uv[0], uv[1]);
                        fillColorRect(image, xx, yy, blockSize, blockSize, pixelData.getColor());
        		}
        		latch.jobDone();
    		};
    		pool.execute(r);
    ...
    public static void renderSceneExecService(Scene scene, Graphics gfx, int width, int height, float resolution) {
    	
    	int blockSize = (int) (1 / resolution);
        long start = System.currentTimeMillis();
        //Latch latch = new Latch(width/blockSize);
        CountDownLatch doneSignal = new CountDownLatch(width/blockSize);
        BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
